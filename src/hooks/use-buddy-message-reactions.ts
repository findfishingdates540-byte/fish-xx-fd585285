...
export function useBuddyMessageReactions(buddyId: string | undefined) {
  const { user } = useAuth();
  const [reactions, setReactions] = useState<Map<string, Reaction[]>>(new Map());
  const [loading, setLoading] = useState(false);

  // Fetch all reactions for messages in this buddy chat
  const fetchReactions = useCallback(async () => {
    ...
    if (error) {
      console.error('Error fetching buddy reactions:', error);
      return;
    }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`buddy-reactions-${buddyId}`)
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'buddy_message_reactions',
        },
        () => {
  ...
  const addReaction = useCallback(async (messageId: string, emoji: string) => {
    ...
    if (error) {
      ...
      if (error.code === '23505') {
        ...
        console.error('Error adding buddy reaction:', error);
      }
    }
  }, [user]);
  ...
  const removeReaction = useCallback(async (messageId: string, emoji: string) => {
    ...
    if (error) {
      console.error('Error removing buddy reaction:', error);
    }
  }, [user]);
