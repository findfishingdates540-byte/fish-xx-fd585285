...
function renderHtml(opts: { name: string; title: string; body: string; ctaLabel?: string | null; ctaUrl?: string | null; }) {
  const bodyHtml = escape(opts.body).replace(/\n/g, "<br/>");
...
<html><body style="margin:0;background:#f5f7fb;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;color:#0b1220;">
  <div style="max-width:600px;margin:0 auto;padding:24px;">
    ...
    <div style="background:#ffffff;border-radius:0 0 16px 16px;padding:28px 24px;border:1px solid #e5e9f2;border-top:0;">
      ...
      <p style="margin:0 0 24px;font-size:15px;line-height:1.6;color:#3a4458;">${bodyHtml}</p>
      ${cta}
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: broadcast, error: bcErr } = await admin
      ...
      .insert({
        title, body: message, channels, audience: "all",
        popup_variant, popup_cta_label, popup_cta_url,
        status: "sending", created_by: userId,
      })
      .select()
      .single();
    ...
    if (channels.includes("in_app")) {
      const rows = allProfiles.map((p: any) => ({
        ...
        title,
        body: message,
        data: { broadcast_id: broadcast.id, cta_url: popup_cta_url, cta_label: popup_cta_label },
      }));
    ...
    const recipientCount = allProfiles.length;
