...
<html><body style="margin:0;background:#f5f7fb;font-family:'Outfit',-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;color:#0b1220;">
  <div style="max-width:560px;margin:0 auto;padding:24px;">
    <div style="background:#031029;color:#fff;border-radius:16px 16px 0 0;padding:24px 28px;text-align:left;">
      <div style="font-weight:700;font-size:22px;letter-spacing:0.5px;">🎣 FishX</div>
      <div style="font-size:12px;opacity:0.7;margin-top:4px;">The angler's network</div>
    </div>
    <div style="background:#ffffff;border-radius:0 0 16px 16px;padding:32px 28px;border:1px solid #e5e9f2;border-top:0;">
      ...
      <a href="${opts.ctaUrl}" style="display:inline-block;background:#1454AE;color:#fff;text-decoration:none;font-weight:600;padding:14px 26px;border-radius:10px;font-size:14px;">${opts.ctaLabel}</a>
      <hr style="border:0;border-top:1px solid #eef1f6;margin:32px 0 18px;">
      <p style="margin:0;font-size:12px;color:#8b93a7;line-height:1.5;">You're receiving this because of your FishX notification settings. <a href="${APP_URL}/app/settings" style="color:#1454AE;">Manage emails</a>.</p>
    </div>
...
serve(async (req) => {
  ...
  try {
    ...
    const html = brandedEmail({
      title: "Your FishX emails look like this",
      intro: "This is a sample of the branded email template every FishX notification, verification, and support email uses.",
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [to],
        subject: "🎣 Your FishX branded email preview",
        html,
      }),
    });
